...
export async function GET(request: NextRequest) {
  try {
    ...
    let orgId = searchParams.get('organization_id') || (user as any)?.user_metadata?.organization_id
    if (!orgId) {
...
export async function POST(request: NextRequest) {
  try {
    ...
    const rateStr = String(body?.rate ?? '').toString().replace('%', '').replace(',', '.').trim()
    const rate = Number(rateStr)
    const synonyms = Array.isArray(body?.synonyms)
      ? body.synonyms.map((s: any) => String(s))
      : String(body?.synonyms || '')
          .split(',').map((s: string) => s.trim()).filter(Boolean)
    if (!code || !label || Number.isNaN(rate)) return NextResponse.json({ error: 'Code, libellé et taux requis (ex: 5.5 ou 5,5)' }, { status: 400 })

    let orgId = String(body?.organization_id || '') || (user as any)?.user_metadata?.organization_id
    if (!orgId) {
      const { data: m } = await (supabaseAdmin as any)
        .from('organization_members')
        .select('organization_id')
    ...
    return NextResponse.json({ success: true, vat: data })
  } catch (e: any) {
    return NextResponse.json({ error: e.message || 'Erreur' }, { status: 500 })
  }
}
